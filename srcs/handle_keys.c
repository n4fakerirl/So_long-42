...
/*   Updated: 2025/08/06 19:15:20 by ocviller         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_onscreen(t_game *game)
{
	...
	str = ft_strjoin("Moves made : ", count);
	if (!str)
		return ;
	mlx_string_put(game->mlx_ptr, game->mlx_win, 10, 10, 0x00FF1493, str);
	free(str);
	free(count);
}
...
int	handle_keypress(int keycode, t_game *game)
{
	...
	if (exit_code == 0)
		loose_screen(game, keycode);
	else if (exit_code == 1)
		win_screen(game, keycode);
	return (0);
}
